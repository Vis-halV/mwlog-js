...
test("escapes fenced code blocks", () => {
  ...
  assert.equal(
    ...
    '<div class="mwlog"><pre><code>console.log(&quot;hello&quot;)</code></pre></div>',
  );
});
...
test("treats unterminated fences as code blocks", () => {
  ...
  assert.equal(
    markdownToHtml(markdown),
    '<div class="mwlog"><pre><code>const x = 1</code></pre></div>',
  );
});
...
test("escapes html inside fenced code blocks", () => {
  ...
  assert.equal(
    markdownToHtml(markdown),
    '<div class="mwlog"><pre><code>&lt;script&gt;alert(1)&lt;/script&gt;</code></pre></div>',
  );
});
